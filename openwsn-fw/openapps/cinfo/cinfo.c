...
// Declaro nuevas funciones
uint16_t get_temp(OpenMote* self);
uint16_t get_humidity(OpenMote* self);
...
owerror_t cinfo_receive(
   ...
   switch (coap_header->Code) {
      case COAP_CODE_REQ_GET:
         ...
         // get random humidity
	 uint16_t humidity = get_humidity(self);
         packetfunctions_reserveHeaderSize(msg,sizeof(humidity)-1);
         memcpy(&msg->payload[0],&humidity,sizeof(humidity)-1);
         
         // get random temperature
	 uint16_t temperature = get_temp(self);
         packetfunctions_reserveHeaderSize(msg,sizeof(temperature)-1);
         memcpy(&msg->payload[0],&temperature,sizeof(temperature)-1);
...
// Funcion que genera temperatura. Utiliza openrandom (de OpenWSN)
uint16_t get_temp(OpenMote* self){
uint16_t mytemp = openrandom_get16b();
return mytemp;
}


// Funcion que genera humedad. Utiliza openrandom (de OpenWSN)
uint16_t get_humidity(OpenMote* self){
uint16_t myhumidity = openrandom_get16b();
